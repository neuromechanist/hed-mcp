...
class TestHEDValidator:
    ...
    async def test_validate_sidecar_file_with_errors(self, validator):
        ...
        # Create temporary sidecar file
        sidecar_data = {"trial_type": {"HED": {"condition_1": "(InvalidTag)"}}}
...
class TestSidecarGenerator:
    ...
    def test_generator_initialization(
        self, generator, mock_schema_handler, mock_tabular_wrapper
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_sidecar_template_success(
        self, generator, mock_tabular_wrapper
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_sidecar_template_failure(
        self, generator, mock_tabular_wrapper
    ):
    ...
    async def test_enhance_template_with_schema(self, generator):
        ...
        assert (
            enhanced["trial_type"]["HED"]["condition_1"]
            == "(Condition/condition-1, Label/condition-1)"
        )
        assert (
            enhanced["trial_type"]["HED"]["condition_2"] == "(Label/existing)"
        )  # Unchanged
    ...
    async def test_suggest_hed_annotation_patterns(self, generator):
        ...
        result = await generator._suggest_hed_annotation(
            "trial_type", "go_trial", mock_schema
        )
        ...
        result = await generator._suggest_hed_annotation(
            "stimulus_type", "visual", mock_schema
        )
        ...
        result = await generator._suggest_hed_annotation(
            "unknown_column", "value", mock_schema
        )
    ...
    async def test_save_sidecar_invalid_format(self, generator):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = await generator.save_sidecar(
                template, output_path, format="unknown"
            )
...
class TestBatchValidator:
    ...
    async def test_validate_directory_success(self, batch_validator, mock_validator):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            events_file1.write_text(
                "onset\tduration\ttrial_type\n1.0\t0.5\tcondition_1\n"
            )
            events_file2.write_text(
                "onset\tduration\ttrial_type\n2.0\t0.5\tcondition_2\n"
            )
            ...
            sidecar_data = {
                "trial_type": {"HED": {"condition_1": "(Label/condition-1)"}}
            }
            ...
            # Validate directory
            results = []
            async for result in batch_validator.validate_directory(
                temp_path, "*_events.tsv"
            ):
    ...
    @pytest.mark.asyncio
    async def test_validate_directory_with_exceptions(
        self, batch_validator, mock_validator
    ):
...
class TestFactoryFunctions:
    ...
    async def test_create_sidecar_generator(
        self, mock_create_wrapper, mock_schema_handler_class
    ):
