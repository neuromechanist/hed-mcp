...
class TestSchemaHandler:
    ...
    def schema_config(self):
        ...
        return SchemaConfig(
            version="8.3.0", fallback_versions=["8.2.0", "8.1.0"], enable_cache=True
        )
    ...
    async def test_load_schema_success(self, schema_handler):
        ...
        with patch(
            "src.hed_tools.hed_integration.schema.load_schema_version",
    ...
    async def test_load_multiple_schemas(self, schema_handler):
        ...
        with patch(
            ...
            result = await schema_handler.load_multiple_schemas(
                ["8.3.0", "8.2.0", "8.1.0"]
            )
    ...
    def test_get_all_schema_tags(self, schema_handler):
        ...
        mock_schema = Mock()
        mock_schema.get_all_tags.return_value = [
            "Event",
            "Sensory-event",
            "Agent-action",
        ]
    ...
    def test_get_tag_descendants(self, schema_handler):
        ...
        mock_schema = Mock()
        mock_schema.get_tag_descendants.return_value = [
            "Event/Sensory-event",
            "Event/Agent-action",
        ]
        schema_handler.schema = mock_schema
    ...
    async def test_preload_schemas(self, schema_handler):
        ...
        with patch.object(
            schema_handler, "load_multiple_schemas"
        ) as mock_load_multiple:
            mock_result = Mock()
...
class TestSchemaManagerFacade:
    ...
    async def test_get_schema(self, facade):
        ...
        with (
            patch.object(facade.handler, "get_schema_info") as mock_info,
...
class TestUtilityFunctions:
    ...
    def test_validate_hed_tag_simple_success(self):
        ...
        with patch(
            "src.hed_tools.hed_integration.schema.HedString",
            return_value=mock_hed_string,
        ):
