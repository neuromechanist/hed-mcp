...
class HEDWrapper:
    ...
    async def load_schema(
        self, version: Optional[str] = None, custom_path: Optional[Path] = None
    ) -> OperationResult:
    ...
    def _analyze_dataframe_columns(self, df: pd.DataFrame) -> List[ColumnInfo]:
        ...
        for col in df.columns:
            ...
            suggested_hed = self._suggest_hed_category(
                col, col_data, is_categorical, is_numeric
            )
    ...
    def _suggest_hed_category(
        self, column_name: str, data: pd.Series, is_categorical: bool, is_numeric: bool
    ) -> Optional[str]:
        ...
        if any(
            term in col_lower
            for term in ["stimulus", "image", "sound", "visual", "auditory"]
        ):
    ...
    async def validate_events(
        self,
        events_data: Union[pd.DataFrame, Path, Dict[str, Any]],
    ...
    async def generate_sidecar_template(
        ...
        try:
            ...
            template = await self._tabular_summary_wrapper.extract_sidecar_template(
                data=events_data, skip_columns=skip_columns, use_cache=use_cache
            )
    ...
    def _enhance_sidecar_template(
        self, base_template: Dict[str, Any], df: pd.DataFrame
    ) -> Dict[str, Any]:
            ...
            if col_info.suggested_hed_category and "HED" not in col_entry:
                if col_info.is_categorical:
                    ...
                    for value in col_info.sample_values:
                        hed_mapping[str(value)] = (
                            f"{col_info.suggested_hed_category}/Label/{value}"
                        )
    ...
    async def analyze_events_data(
        ...
        required_columns_present = all(
            col in df.columns for col in ["onset", "duration"]
        )
...
def create_hed_wrapper(
    schema_version: str = "8.3.0", config: Optional[HEDWrapperConfig] = None
) -> HEDWrapper:
...
class HEDIntegration:
    ...
    async def validate_events_file(
        self, events_file: Path, sidecar_file: Optional[Path] = None
    ) -> Dict[str, Any]:
