...
try:
    ...
    class HedValidator:
        ...
        def validate_string(self, hed_string: str):
            return []
    ...
    class TabularInput:
        ...
        def validate_file(self, schema):
            return []

    def get_printable_issue_string(issues):
        return str(issues)
...
from .models import ValidationResult, SidecarTemplate, OperationResult
...
class HEDValidator:
    ...
    async def validate_string(
        self, hed_string: str, schema_version: Optional[str] = None
    ) -> ValidationResult:
    ...
    async def validate_events_data(
        self,
        events_data: Union[pd.DataFrame, Path, str],
    ...
    async def validate_sidecar_file(
        self, sidecar_path: Union[Path, str], schema_version: Optional[str] = None
    ) -> ValidationResult:
                    ...
                    if isinstance(hed_info, dict):
                        ...
                        for value, hed_string in hed_info.items():
                            if isinstance(hed_string, str):
                                result = await self.validate_string(
                                    hed_string, schema_version
                                )
...
class SidecarGenerator:
    ...
    async def _enhance_template_with_schema(
        self, template: Dict[str, Any], schema_version: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def _suggest_hed_annotation(
        self, column_name: str, value: str, schema
    ) -> Optional[str]:
...
class BatchValidator:
    ...
    def __init__(self, validator: Optional[HEDValidator] = None, max_workers: int = 4):
    ...
    async def validate_directory(
        self, directory: Path, pattern: str = "*.tsv", sidecar_pattern: str = "*.json"
            ...
            for events_file in chunk:
                ...
                if sidecar_file.exists():
                    try:
                        ...
                        self.logger.warning(
                            f"Could not load sidecar {sidecar_file}: {e}"
                        )
    ...
    async def _validate_single_file(
        self, events_file: Path, sidecar: Optional[Dict]
    ) -> ValidationResult:
...
class BIDSValidator:
    ...
    async def validate_bids_dataset(
        self, dataset_path: Path, validate_hed: bool = True
    ) -> Dict[str, Any]:
...
async def create_sidecar_generator(
    schema_version: Optional[str] = None, **tabular_summary_config
) -> SidecarGenerator:
...
async def create_batch_validator(
    schema_version: Optional[str] = None, max_workers: int = 4
) -> BatchValidator:
