...
class SchemaConfig(BaseModel):
    ...
    custom_path: Optional[Path] = Field(
        default=None, description="Path to custom schema file"
    )
    ...
    auto_update: bool = Field(
        default=True, description="Automatically update schema cache"
    )
...
class ValidationConfig(BaseModel):
    ...
    check_required_tags: bool = Field(
        default=True, description="Check for required tags"
    )
...
class TabularSummaryConfig(BaseModel):
    ...
    value_columns: Optional[List[str]] = Field(
        default=None, description="Specific columns to treat as value columns"
    )
    name: str = Field(default="", description="Name for the summary")
    include_description: bool = Field(
        default=True, description="Include descriptions in output"
    )
    max_unique_values: int = Field(
        default=50, description="Maximum unique values to include"
    )
...
class HEDWrapperConfig(BaseModel):
    ...
    async_timeout: float = Field(
        default=30.0, description="Default timeout for async operations"
    )
...
class ColumnInfo(BaseModel):
    ...
    suggested_hed_category: Optional[str] = Field(
        default=None, description="Suggested HED category"
    )
...
class EventsData(BaseModel):
    ...
    dataframe: Optional[pd.DataFrame] = Field(
        default=None, description="Events DataFrame"
    )
    columns: List[ColumnInfo] = Field(
        default_factory=list, description="Column information"
    )
    row_count: int = Field(description="Number of rows in the data")
    required_columns_present: bool = Field(
        description="Whether onset/duration are present"
    )
...
class SchemaInfo(BaseModel):
    ...
    library_schemas: List[str] = Field(
        default_factory=list, description="Associated library schemas"
    )
...
class BatchProcessingConfig(BaseModel):
    ...
    continue_on_error: bool = Field(
        default=True, description="Continue processing if one item fails"
    )
    timeout_per_item: float = Field(
        default=60.0, description="Timeout per item in batch"
    )
