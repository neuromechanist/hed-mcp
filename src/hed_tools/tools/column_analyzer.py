...
class BIDSColumnAnalyzer:
    ...
    async def analyze_events_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Analysis completed: {len(analysis['hed_candidates'])} HED candidates found"
            )
    ...
    def _is_column_hed_suitable(
        ...
        if analysis["type"] == "numeric" and column_name.lower() in [
            "response_time",
            "reaction_time",
            "accuracy",
            "score",
        ...
        if (
            ...
            and analysis["unique_count"] <= 10
            and analysis["unique_count"] >= 2
        ):
            return True
        ...
        return (
            analysis["type"] in ["categorical", "text"]
            and analysis["unique_count"] >= 2
        )
        ...
        for col in df.columns:
            if not col.islower() or " " in col:
                compliance["warnings"].append(
                    f"Column '{col}' should be lowercase with no spaces"
                )
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if not analysis["bids_compliance"]["valid"]:
            recommendations.append(
                "Address BIDS compliance issues before HED annotation"
            )
        ...
        for col_name, col_info in analysis["columns"].items():
            if col_info["null_count"] > 0:
                recommendations.append(
                    f"Consider handling {col_info['null_count']} null values in '{col_name}'"
                )

        if not hed_candidates:
            recommendations.append(
                "No suitable columns found for HED annotation - check column types and values"
            )

        return recommendations

    async def suggest_hed_annotations(
        self, column_data: pd.Series, column_name: str = None
    ) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for value in unique_values[:20]:  # Limit to 20 suggestions
                suggestion = {
                    "value": str(value),
                    "suggested_hed": self._suggest_hed_for_value(
            ...
            logger.info(
                f"Generated {len(suggestions)} HED suggestions for column '{column_name}'"
            )
            return suggestions
