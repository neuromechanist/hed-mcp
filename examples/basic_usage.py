...
async def create_sample_events_file(output_path: Path) -> bool:
    ...
    events_data = {
        ...
        'trial_type': ['face', 'house', 'face', 'house', 'scrambled', 'face', 'house', 'scrambled', 'face', 'house'],
        'condition': ['happy', 'modern', 'sad', 'old', 'pattern', 'neutral', 'modern', 'pattern', 'happy', 'old'],
        'response': ['left', 'right', 'left', 'left', 'right', 'right', 'left', 'right', 'left', 'right'],
...
async def demonstrate_hed_integration(events_path: Path, analysis: dict):
    ...
    if analysis and analysis['hed_candidates']:
        ...
        if events_df is not None:
            ...
            for candidate in analysis['hed_candidates'][:3]:  # Limit to first 3 for demo
                ...
                for suggestion in suggestions[:5]:  # Show first 5 suggestions
                    confidence_icon = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(suggestion['confidence'], "⚪")
                    print(f"   {confidence_icon} {suggestion['value']} → {suggestion['suggested_hed']}")
...
async def demonstrate_sidecar_generation(events_path: Path, analysis: dict):
    ...
    if sidecar:
        ...
        for column, content in sidecar.items():
            ...
            if 'HED' in content:
                print(f"   HED annotations:")
                hed_content = content['HED']
                if isinstance(hed_content, dict):
                    for value, hed_string in list(hed_content.items())[:3]:  # Show first 3
...
async def demonstrate_quick_analysis(events_path: Path):
    ...
    try:
        ...
        print(f"   BIDS compliant: {results.get('bids_compliance', {}).get('valid', False)}")
